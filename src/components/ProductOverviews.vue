    ...
    <div class="pt-6">
      ...
      <div class="max-w-2xl mx-auto mt-6 sm:px-6 lg:max-w-7xl lg:px-8 lg:grid lg:grid-cols-3 lg:gap-x-8">
        <div class="hidden overflow-hidden rounded-lg aspect-w-3 aspect-h-4 lg:block">
          <img :src="product.images[0].src" :alt="product.images[0].alt" class="object-cover object-center w-full h-full" />
        </div>
        <div class="hidden lg:grid lg:grid-cols-1 lg:gap-y-8">
          <div class="overflow-hidden rounded-lg aspect-w-3 aspect-h-2">
            <img :src="product.images[1].src" :alt="product.images[1].alt" class="object-cover object-center w-full h-full" />
          </div>
          <div class="overflow-hidden rounded-lg aspect-w-3 aspect-h-2">
            <img :src="product.images[2].src" :alt="product.images[2].alt" class="object-cover object-center w-full h-full" />
          </div>
        </div>
        <div class="aspect-w-4 aspect-h-5 sm:rounded-lg sm:overflow-hidden lg:aspect-w-3 lg:aspect-h-4">
          <img :src="product.images[3].src" :alt="product.images[3].alt" class="object-cover object-center w-full h-full" />
        </div>
      </div>
      ...
      <div class="max-w-2xl mx-auto pt-10 pb-16 px-4 sm:px-6 lg:max-w-7xl lg:pt-16 lg:pb-24 lg:px-8 lg:grid lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8">
        ...
        <div class="mt-4 lg:mt-0 lg:row-span-3">
          ...
          <form class="mt-10">
                ...
                <div class="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                  <RadioGroupOption as="template" v-for="size in product.sizes" :key="size.name" :value="size" :disabled="!size.inStock" v-slot="{ active, checked }">
                    <div :class="[size.inStock ? 'bg-white shadow-sm text-gray-900 cursor-pointer' : 'bg-gray-50 text-gray-200 cursor-not-allowed', active ? 'ring-2 ring-indigo-500' : '', 'group relative border rounded-md py-3 px-4 flex items-center justify-center text-sm font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1 sm:py-6']">
                      ...
                      <div v-else aria-hidden="true" class="absolute border-2 border-gray-200 rounded-md pointer-events-none -inset-px">
            ...
            <button type="submit" class="flex items-center justify-center w-full px-8 py-3 mt-10 text-base font-medium text-white bg-indigo-600 border border-transparent rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Add to bag</button>
          </form>
        </div>
